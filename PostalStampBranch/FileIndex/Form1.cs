...
namespace FileIndex
{
    public partial class Form1 : Form
    {
        ...
        private bool IsFormValid()
        {
            ...
            // 2. Stamp Check (Har Type ke liye lazmi)
            if (stampPrice.Value == 0 || stampDesignNUM.Value == 0)
            {
                MessageBox.Show("Stamp Price and Design Number are mandatory.");
                return false;
            }

            if (stampQtyFC.Value == 0 || stampQty.Value == 0)
            {
                MessageBox.Show("Free of Cast Stamp and Stamp Printing Qty are mandatory!");
                return false;
            }

            // 3. FDC aur Leaflet Check (Ye bhi ab har Type ke liye lazmi hain)
            if (FDCPrice.Value == 0 || leafletPrice.Value == 0 || FDCQty.Value == 0 || leafletQty.Value == 0)
            {
                MessageBox.Show("FDC and Leaflet details are mandatory!");
                return false;
            }

            // 4. Souvenir Check (Sirf Type 2 ke liye lazmi)
            if (currentFileType == 2)
            {
                if (souvenirPrice.Value == 0 || souvenirDesignNUM.Value == 0)
                {
                    MessageBox.Show("Souvenir Price and Design NO are mandatory for this File Type!");
                    return false;
                }
            }

            return true; // Agar sab upar wale tests pass ho gaye
        }
