...
namespace FileIndex
{
    ...
    {
        public void DispatchTypeLoad( ComboBox cmb)
        { 
            using (SqlConnection con = new (Db.ConString))
               ...
               SqlDataAdapter da= new(query, con);
                ...
                da.Fill(dt);
                cmb.DataSource = dt;
                cmb.DisplayMember = "DispatchType";
                cmb.ValueMember = "ID";
                cmb.SelectedIndex = -1;
            }
        }
        ...
        private void exportBtn_Click(object sender, EventArgs e)
        {
            ...
            if (com_FileNo.SelectedIndex == -1|| cmb_DT.SelectedIndex==-1)
            {
                MessageBox.Show("Please select Invoice and DispatchType Number.");
                return;
            }
            ...
            INNER JOIN StockPrice PR ON PR.FileNo = F.Id
            ...
            ORDER BY I.InvoiceNo ASC";
                ...
                da.SelectCommand.Parameters.AddWithValue("@dt", cmb_DT.ValueMember);
        ...
        private void searchBtn_Click(object sender, EventArgs e)
        {
           ...
           INNER JOIN FileIndex F ON F.Id = I.FileNo
            LEFT JOIN PhilitelicBuearu P ON P.Id = I.PhiliticBureauName -- Yahan column name check karein
            LEFT JOIN PendingInvoice PI ON PI.InvoiceRegisterId = I.ID -- Join hamesha Common ID par hota hai
            LEFT JOIN AcknowldeType A ON PI.AcknowledgeStatus = A.ID -- Table ka sahi naam likhein
            WHERE (@id = 3 AND PI.AcknowledgeStatus IN (1, 2)) 
               OR (PI.AcknowledgeStatus = @id)
            ORDER BY I.InvoiceNo ASC";
