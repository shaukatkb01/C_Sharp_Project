    ...
    public partial class IssuePrinting : Form
    {
        ...
        private void IssuePrinting_Load(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                ...
                FormLoadingData.SignatureAuthority(cmb_Signature, cmb_BPS);
        ...
        private void btn_Distribution_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Select an Issue Number.");
                return;
            }
            ...
            using (var con = new SqlConnection(Db.ConString))
            {
                ...
                {
                    MessageBox.Show("No Distribution record found for this issue.");
                }
            }
...
AND PS.Address NOT IN(24,25)";
                ...
                {
                    MessageBox.Show("Pahly Distribution List ki entry kary (No data found for this Issue)." + issueId.ToString());
                }
            }
        }
        ...
        private void btn_OffierAddres_Click(object sender, EventArgs e)
        {
            if (cmb_BPS.SelectedIndex == -1)
            {
                MessageBox.Show("Select BPS first");
                cmb_BPS.Focus();
                cmb_BPS.DroppedDown = true;
                return;
            }

            string printOption = "";

            // 1. Pehle sirf option wala form khol kar band karein
            using (AddressOption optForm = new AddressOption())
            {
                if (optForm.ShowDialog() == DialogResult.OK)
                {
                    printOption = optForm.SelectedOption;
                }
                else
                {
                    return; // Agar cancel kiya to yahin se wapis chale jayein
                }
            } // <--- Form ka kaam yahan khatam

            // 2. Ab database ka kaam shuru karein (Option form band hone ke BAAD)
            int bpsId = Convert.ToInt32(cmb_BPS.SelectedValue);
            ...
            using (SqlConnection con = new SqlConnection(Db.ConString))
            {
                try
                {
                    ...
                    string query = @"SELECT 
                                ...
                                P.Address, 
                                B.BPS
                               FROM PhilitelicBuearu P
                               LEFT JOIN BPS B ON P.BPS = B.BPSID
                               WHERE BPSID=@id";

                    SqlDataAdapter da = new SqlDataAdapter(query, con);
                    da.SelectCommand.Parameters.AddWithValue("@id", bpsId);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        // RDLC parameter pass karein
                        reportParams.Add(new ReportParameter("p_PrintType", printOption));

                        reportForm.LoadReport(dt, "dtOfficerAddress", "Report/rptOfficerAddress.rdlc", reportParams);
                        reportForm.Show();
                    }
                    else
                    {
                        MessageBox.Show("Is BPS kay koy Address nahi mila.");
                    }
                }
                catch (Exception ex)
                {
