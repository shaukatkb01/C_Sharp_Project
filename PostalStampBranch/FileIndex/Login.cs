...
namespace FileIndex
{
    ...
    public partial class Login : Form
    {
        ...
        {
            ThemeManager.ApplyTheme(this); // Form par theme apply karo
        }
     ...
     string.IsNullOrWhiteSpace(passwordTxt.Text))
        ...
        {  try
            {
                // 1. Register form ka object banayein
                using (Register regForm = new Register())
                {
                    regForm.StartPosition = FormStartPosition.CenterScreen;

                    // 2. Login ko hide karne ki zaroori nahi agar aap ShowDialog use karein
                    // Lekin agar hide karna chahte hain toh:
                    this.Hide();

                    // 3. ShowDialog use karein. Yeh code ko yahin rok dega jab tak Register band na ho
                    regForm.ShowDialog();

                    // 4. Jab Register form band hoga, toh control wapis yahan aayega
                    this.Show();
                    this.Activate();
                }
            }
