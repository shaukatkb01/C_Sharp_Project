...
namespace FileIndex
{
    public partial class DistributionList : Form
    {
        ...
        {
            ComboBoxHelper.SetLeftDropdown(combFileNo, 400);
        ...
        // 1. Yeh naya function add karein (Select on focus)
        private void SelectOnEnter(object sender, EventArgs e)
        {
        ...
        // 2. Apne AttachEventsToAllNumericControls ko is tarah update karein:
        private void AttachEventsToAllNumericControls(Control parent)
        {
            foreach (Control c in parent.Controls)
            {
                ...
                {
                    // 1. Jab mouse ya Tab se cursor ComboBox par aaye
                    combo.Enter += (s, ev) =>
                    {
                        combo.DroppedDown = true; // List ko niche gira do (Auto-open)
                    };

                    // 2. Agar selection change hone par stock update karna hai
                    if (combo.Name.StartsWith("drop_"))
                    {
                ...
                if (c is NumericUpDown numControl)
                {
                    // A. Purana selection wala function yahan attach karein
                    numControl.Enter += SelectOnEnter;

                    // B. Calculation wala logic (sirf specific controls ke liye)
                ...
                // Agar control ke andar mazeed controls hain (jaise panel2)
