...
namespace FileIndex
{
    internal class FormLoadingData
    {
        ...
        public static void SignatureAuthority(ComboBox cmb, ComboBox cmb2)
        {
            using (var con = new SqlConnection(Db.ConString))
            {
                try
                {
                    ...
                    string query2 = @"SELECT 
                                    ...
                                    AND BPSID <=22
                                    ORDER BY BPSID ASC";
                    SqlDataAdapter adapter1 = new(query2, con);
                    DataTable dt2 = new();
                    adapter1.Fill(dt2);
                    cmb2.DataSource = dt2;
                    cmb2.DisplayMember = "BPS";
                    cmb2.ValueMember = "BPSID";
                    cmb2.SelectedIndex = -1; // Shuru mein kuch select na ho
