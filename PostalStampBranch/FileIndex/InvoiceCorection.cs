...
namespace FileIndex
{
    public partial class InvoiceCorection : Form
    {
            ...
            using (SqlConnection con = new SqlConnection(Db.ConString))
            {
                ...
                try
                {
                    string query1 = @"UPDATE InvoiceRegister
                                    ...
                                    Totalamount=@ta,Remarks=@remarks
                                    WHERE Id=@in";
        ...
        private void com_InvoiceNo_SelectedIndexChanged_1(object sender, EventArgs e)
        {
                ...
                using (SqlConnection con = new SqlConnection(Db.ConString))
                {
                    try
                    {
                        ...
                        string query = @"SELECT 
                                ...
                                LEFT JOIN StockPrice P ON IR.FileNo = P.FileNo                 -- Link 2
                                WHERE IR.Id = @id
                                AND PS.SupplyType IN (2,4)";
