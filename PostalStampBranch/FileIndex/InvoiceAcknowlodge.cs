...
namespace FileIndex
{
    public partial class InvoiceAcknowlodge : Form
    {
        ...
        private void LoadDataToGrid(DataGridView targetGrid, int acknowledgeType)
        {
            using (SqlConnection con = new SqlConnection(Db.ConString))
            {
                ...
                string query = @"SELECT 
                    ...
                    FORMAT(I.Totalamount, 'N0') AS Totalamount, 
                    A.AcknowledgeType, 
                ...
                try
                {
                    ...
                    {
                        // Pehle AutoSize band karein taake Manual width kaam kare
                        targetGrid.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.None;

                        if (targetGrid.Columns.Contains("Id"))
                            targetGrid.Columns["Id"].Visible = false;

                        if (targetGrid.Columns.Contains("PhilitelicBuearuName"))
                        {
                            targetGrid.Columns["PhilitelicBuearuName"].Width = 300;
                            // Agar aap chahte hain ke ye column bachi hui sari jagah le le:
                            // targetGrid.Columns["PhilitelicBuearuName"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                        }

                        if (targetGrid.Columns.Contains("Totalamount"))
                        {
                            targetGrid.Columns["Totalamount"].Width = 120;
                            targetGrid.Columns["Totalamount"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                        }

                        if (targetGrid.Columns.Contains("InvoiceNo"))
                            targetGrid.Columns["InvoiceNo"].Width = 150;
                            targetGrid.Columns["InvoiceNo"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                    }
                }
                catch (Exception ex)
                {
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("First select a record from the list!");
                return;
            }
            if (radio_Pending.Checked && string.IsNullOrWhiteSpace(txt_PageNo.Text))
            {
            ...
            using (SqlConnection con = new SqlConnection(Db.ConString))
            {
                ...
                else if (radio_Pending.Checked|| !string.IsNullOrWhiteSpace(txt_PageNo.Text))
                {
                    cmd.Parameters.AddWithValue("@rd", Picker_RDate.Value.Date);
                    cmd.Parameters.AddWithValue("@as", 1);
                    InvoiceToComboBox(cmb_InvoiceNo, 1);
