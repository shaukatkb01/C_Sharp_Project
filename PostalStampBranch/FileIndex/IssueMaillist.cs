...
namespace FileIndex
{
    public partial class IssueMaillist : Form
    {
        ...
        private void EnableDisablePhil(Control parent, bool status)
        {
            ...
            {
                // Tag check karein
                if (c.Tag != null && c.Tag.ToString() == "Phil")
                {
                    c.Enabled = status;
                }

                // Agar is control ke andar mazeed controls hain (jaise GroupBox ya Panel)
                if (c.HasChildren)
                {
                    EnableDisablePhil(c, status); // Function dubara khud ko call karega
                }
            }
        }
